...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf_token(post('csrf_token'))) {
    ...
    if (empty($errors)) {
        ...
        set_flash('success', 'Service saved.');
...
$pageTitle = 'Services — Admin — ' . APP_NAME;
require_once __DIR__ . '/../includes/header.php';
?>

<div class="container">
    <h1 class="page-title mt-4 mb-3">Manage Services</h1>
    ...
    <div class="card section-card p-4 mb-4">
        <h5 class="fw-bold mb-3"><?= $editService ? 'Edit Service' : 'Add New Service' ?></h5>
        <form method="post" novalidate>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="slug" class="form-control" value="<?= e($editService['slug'] ?? post('slug')) ?>" placeholder="auto-generated if blank">
                </div>
                <div class="col-12">
                ...
                <div class="col-md-4">
                    ...
                    <select name="is_active" class="form-select">
                        <option value="1" <?= ($editService['is_active'] ?? 1) == 1 ? 'selected' : '' ?>>Yes</option>
                        <option value="0" <?= ($editService['is_active'] ?? 1) == 0 ? 'selected' : '' ?>>No</option>
                    </select>
                </div>
            </div>
            <button class="btn btn-primary mt-3 rounded-pill"><i class="bi bi-save me-2"></i><?= $editService ? 'Update Service' : 'Add Service' ?></button>
            <?php if ($editService): ?>
                <a href="services.php" class="btn btn-outline-secondary mt-3 ms-2 rounded-pill">Cancel</a>
            <?php endif; ?>
        </form>
    </div>
